...
/**
 * The Class SqliteVisitor. It creates a SQL statement string from a SQL tree.
 */
public class SqliteVisitor implements Visitor {
	...
	public String getGeneratedCommand(){
		...
		ret += "SELECT "+select;
		ret += " FROM "+from;
		if( where != null )ret += " WHERE "+where;
		if( groupby != null )ret += "GROUP BY "+groupby;
		return ret;
	}
	...
	/**
	 * It saves the source table names.
	 *
	...
	/**
	 * The Class ExpressionVisitor.  Used to generate a SQL statement from a math/bool expression
	 */
